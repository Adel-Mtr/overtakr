...
Verified on 10 September 2026: the hosted frontend loaded race and driver data and completed a 2024 Emilia Romagna strategy comparison, including charts, leaderboard, driver digest and position-change insights.
...
2. Confirm the actual assigned public HTTPS URLs; do not guess them from service names. The current Blueprint contains the verified URLs of the existing demo. If deploying a separate copy, replace them with that copy's actual service origins.
...
Keep the three URL values in `render.yaml` consistent with the service environment variables before syncing the Blueprint again. A healthy build alone is not evidence that analysis works.
